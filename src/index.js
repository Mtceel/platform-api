...
const authMiddleware = async (req, res, next) => {
  ...
  if (apiGatewayTenantId) {
    ...
    // Skip audit log (geen user_id in users table)
    return next();
  }
...
app.get('/api/admin/kubernetes', authMiddleware, superAdminMiddleware, async (req, res) => {
  try {
    // Mock Kubernetes data - in production zou dit kubectl gebruiken
    const pods = [
      { name: 'platform-api', status: 'Running', restarts: 0, age: '2h' },
      { name: 'postgres', status: 'Running', restarts: 0, age: '45h' },
      { name: 'redis', status: 'Running', restarts: 0, age: '45h' },
      { name: 'storefront-renderer', status: 'Running', restarts: 0, age: '30m' },
      { name: 'api-gateway', status: 'Running', restarts: 0, age: '15m' },
    ];
    
    res.json({ pods });
  } catch (err) {
    console.error('Kubernetes error:', err);
    res.status(500).json({ error: 'Failed to fetch kubernetes data' });
  }
});
...
app.get('/api/products', authMiddleware, async (req, res) => {
  try {
    const result = await db.query(
      'SELECT id, name, description, price, stock, featured, created_at FROM products WHERE tenant_id = $1 ORDER BY created_at DESC',
      [req.tenantId]
    );
...
app.patch('/api/products/:id', authMiddleware, async (req, res) => {
  ...
  if (featured === undefined) {
    return res.status(400).json({ error: 'Featured field is required' });
  }
  
  try {
    const result = await db.query(
      'UPDATE products SET featured = $1 WHERE id = $2 AND tenant_id = $3 RETURNING *',
      [featured, id, req.tenantId]
    );
    ...
    console.error('Update product featured error:', err);
...
app.get('/api/storefront/:subdomain', async (req, res) => {
  ...
  try {
    ...
    const productsResult = await db.query(
      'SELECT id, title as name, description, price, inventory as stock, image_url, category, tags FROM products WHERE tenant_id = $1 AND status = $2 ORDER BY created_at DESC',
      [tenantId, 'active']
    );
    ...
    res.json({
      name: storeResult.rows[0].store_name,
      subdomain: storeResult.rows[0].subdomain,
...
app.get('/api/theme/active', authMiddleware, async (req, res) => {
  try {
    const result = await db.query(
      'SELECT * FROM themes WHERE tenant_id = $1 AND is_active = true LIMIT 1',
      [req.tenantId]
      ...
      const defaultTheme = {
        ...
        sections: [
          ...
          },
          {
            ...
            settings: {
              title: 'About Us',
              content: 'We are a modern e-commerce store.'
            }
          }
        ]
      };
      
      const newTheme = await db.query(
        'INSERT INTO themes (tenant_id, name, settings, is_active) VALUES ($1, $2, $3, $4) RETURNING *',
        [req.tenantId, 'Default Theme', defaultTheme, true]
      );
      
      return res.json({ theme: newTheme.rows[0] });
    }
...
// Update theme settings (live save)
app.put('/api/theme/active', authMiddleware, async (req, res) => {
  const { settings } = req.body;
  
  try {
    const result = await db.query(
      'UPDATE themes SET settings = $1, updated_at = NOW() WHERE tenant_id = $2 AND is_active = true RETURNING *',
      [settings, req.tenantId]
    );
    ...
    await redis.del(`storefront:theme:${req.tenantId}`);
